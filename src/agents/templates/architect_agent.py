...
class ArchitectAgent(AbstractAgent):
    ...
    async def execute_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            project_id = task_data.get("project_id", f"proj_{datetime.now(timezone.utc).timestamp()}")
    ...
    def _analyze_complexity(self, spec: Dict[str, Any]) -> int:
        ...
        # Types de taches
        task_types = [t.get("type", "").lower() for t in tasks if isinstance(t, dict)]
        ...
        # Dependances
        has_dependencies = any(t.get("dependencies") for t in tasks if isinstance(t, dict))
        if has_dependencies:
            score += 1
    ...
    async def generate_dag(
        ...
        for i, task_spec in enumerate(tasks):
            ...
            # Extraction des dependances (robustesse)
            deps = task_spec.get("dependencies") or []
            ...
            node = TaskNode(
                ...
                dependencies=deps,
                parameters=task_spec.get("parameters", {}) or {},
    ...
    def _extract_skills(self, spec: Dict[str, Any], dag: List[TaskNode]) -> Set[str]:
        ...
        # 0. Competences de niveau racine
        ...
        if isinstance(team_reqs, list):
            for req in team_reqs:
                if isinstance(req, dict):
                    skill = req.get("skill") or req.get("name")
                    if skill:
                        skills.add(skill)
        ...
        for node in dag:
            implicit_skills = self._get_implicit_skills(node.task_type.value)
    ...
    def _extract_task_skills(self, task_spec: Dict[str, Any]) -> List[str]:
        ...
        """
        skills = task_spec.get("skills_required", []) or []
    ...
    async def _create_missing_skills(
        ...
        for skill_id in missing_skills:
            try:
                ...
                if self.llm_client:
                    skill_class = await self._synthesize_skill(skill_id, spec)
                    if skill_class:
        ...
        try:
            ...
            class DynamicSkill(BaseSkill):
                ...
                def get_system_prompt_rules(self) -> str:
                    return f"Execute the {skill_id} skill."
